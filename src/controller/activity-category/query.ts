...
export const findOneById = async (id: number) => {
    let query = `select * from ${table} where id_category_activity = ?`;
...
export const updateById = async (id: number, doc: ActivityCategory) => {
	...
	param.push(id);
	let query = `update ${table} set ${set.join(',')} where id_category_activity = ?`;
	const result = await execute(query,param);
	return result;
}

export const remove = async (id: number) => {
    let query = `delete from ${table} where id_category_activity = ?`;
	const result = await execute(query,[id]);
	return result;
}
