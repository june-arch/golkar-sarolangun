...
export const findAllPagination = async (page: number, limit: number, search:string) => {
    let query = `select * from ${table} where is_deleted=0 ${search && 'and fullname like "%'+ search+'%"'} order by ? desc limit ? offset ?`;
	const result = await execute(query,['created_date', limit, (page-1)*limit]);
	return result;
}

export const countAll = async (search) => {
    let query = `select count(*) as count from ${table} where is_deleted=0 ${search && 'and fullname like "%'+ search+'%"'}`;
	const result = await execute(query,[]);
	return result;
}
