...
export const findAllPaginationHome = async (
  ...
  let query = `SELECT a.id_activity, b.name AS category, a.title, a.created_date, a.image, a.video 
  FROM activity 
  a LEFT JOIN category_activity b ON a.category_activity_id = b.id_category_activity 
  ${search && 'where'} ${search && 'title like "%' + search + '%"'}
