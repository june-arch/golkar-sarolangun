...
import { getAllPagination, registerMember } from '@/controller/member/domain'
...
handler.use(jwt)
  ...
  .post(validate({ body: createMember }), async (req: NextApiRequestModify, res: NextApiResponse) => {
    ...
    const sendResponse = async (result) => {
      return (result.err) ? wrapper.response(res, 'failed', result, 'create new member kta')
        : wrapper.response(res, 'success', result, 'create new member kta', 201);
    };
