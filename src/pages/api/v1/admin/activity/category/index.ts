...
import { createCategory, getAllPagination } from '@/controller/activity-category/domain'
...
handler
  ...
  .get(async (req, res) => {
    ...
    const sendResponse = async (result) => {
      return (result.err) ? wrapper.response(res, 'failed', result, 'get all activity category')
        : wrapper.responsePage(res, 'success', result, 'get all activity category', 200);
    };
  ...
  .post(async (req, res) => {
    ...
    const sendResponse = async (result) => {
      return (result.err) ? wrapper.response(res, 'failed', result, 'get activity category')
        : wrapper.response(res, 'success', result, 'create activity category', 200);
    };
